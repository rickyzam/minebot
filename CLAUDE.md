...
```bash
npm test                  # 435 unit tests. No network, no model. Fast. Run these constantly.
npm run typecheck         # Whole repo, including scripts/.
npm run test:integration  # 141 tests, NO SKIPS. Requires the live dev server. The last skip — the contract suite's `flee` guarantee — closed when Task 6b landed and `prepareFleeFixture` was supplied
...
**`npm run demo:phase5` used to go red for a reason that was not a regression, and that cause is now GONE.** MEASURED 2026-09-12 across six runs while `flee` was a stub: `done`, `stuck`, **DIED**, `done`, **DIED**, `done` — two deaths in six, both because three flee triggers fired as health fell 5.7 → 4.2 and each returned `internal: flee arrives in Phase 5`, so nothing evaded the zombie. **Task 6b landed 2026-09-14 and `flee` now runs**, so a flee trigger does something. If this demo dies again, that is a real finding about `flee` under load rather than a known gap — read the preemption list before assuming either way.
...
| **The bot's top travel speed is 5.60 blocks/sec** — MEASURED 2026-09-13 on a flat, straight, unobstructed 90-block runway: a 20-block leg took 3.62s (5.49 b/s) and a 60-block leg 10.57s (5.60 b/s) | That is Minecraft's sprint speed (5.612 m/s), so it is a CEILING, not a tuning target. Any "go N blocks in T seconds" requirement has to clear N/5.6 seconds or it cannot be met. This is what showed `flee`'s proposed "100 blocks or 10s" to be self-contradictory — 100 needs ≥17.9s, and 10s buys ~56 blocks at best — and it is why the 100 is a target rather than a requirement |
| **A pathfinder goal the bot cannot path to means it does not move AT ALL** — not that it moves partway | Bit `flee`'s design: a single ring of candidate destinations at the agreed 100 blocks would put every candidate off the edge of any confined floor, turning "I could have run 30 blocks that way" into `unreachable`. `flee` therefore tries a LADDER of radii (100, 64, 32, 16, 8) and takes the furthest reachable one. The same trap waits for anything that picks a distant goal and assumes partial progress |
| **Entities do not obstruct the pathfinder** | MEASURED 2026-09-14 while building `flee`'s `unreachable` fixture: a 1×1 pocket walled on four sides with a zombie standing in the only opening is NOT sealed — the bot walked straight past the mob and fled successfully. A fixture that needs the bot boxed in must wall every side and put the hostile OUTSIDE; the mob is never the wall. Note it is still SEEN from in there, because `nearbyEntities` is distance-based rather than line-of-sight |
...
- **Both are systemd user units. Use them; do not hand-start either.**
  ```bash
  systemctl --user start mc          # the backend. Down after every boot, ON PURPOSE
  systemctl --user stop mc           # sends the console `stop` and WAITS for the flush
  systemctl --user status mc velocity
  ```
  `~/.config/systemd/user/{mc,velocity}.service`, with `loginctl enable-linger zman` so units run at boot rather than at login. **No root is needed for any of this** — user units and self-lingering are both unprivileged. The units are checked in at [`ops/systemd/`](ops/systemd/) with install steps and the reasoning; edit there and copy across, or the next machine rebuild loses them.
- **The proxy autostarts; the backend deliberately does not.** The proxy is stateless — no world, no player data — so restarting it costs nothing. A world has to be flushed on the way down, so bringing one up stays a deliberate act, and integration tests assume they own the arena. `mc.service` therefore has **no `[Install]` section**: it never starts at boot.
- **What `mc.service` is actually for is the SHUTDOWN.** systemd stops every *active* unit during shutdown whether or not it is enabled, and it **waits** for `ExecStop`. So a reboot now flushes the world instead of killing it. That was measured to be a real problem, not a hypothetical: **three of the last seven runs (Sep 8, 9, 11) ended mid-activity with no `Stopping server`, no `Saving worlds` and no `All dimensions are saved`** — roughly half of recent runs never flushed, because a hand-started tmux session gets no ordered shutdown. VERIFIED 2026-09-12: `systemctl --user stop mc` logged `Stopping server → Saving players → Saving worlds → All chunks are saved ×3 → All dimensions are saved` in 1.06s.
- **`mc.service` refuses to start if anything already listens on 25566**, and otherwise clears a leftover `mc` tmux session first. Both guards are measured, not theoretical: the unit failed its own first start with `duplicate session: mc`, because stopping a hand-started server leaves the session alive at a shell prompt. The refusal exists so two servers can never write one world — VERIFIED firing, and it did **not** touch the live session when it did.
- **A stopped backend now names its own fix.** `requireBackend()` (a TCP check on 25566) runs as the integration project's `globalSetup` and at the top of `smoke` and every `demo:*`, so a down server gives one line — *"start it with: systemctl --user start mc"* — instead of 134 connection failures that each read as a code bug. It deliberately does **not** start the server: a test run must not bring up shared infrastructure nobody asked for. It only proves the port is open; `npm run smoke` is what proves a bot can complete the Fabric and Velocity handshakes.
...
- Use `stop` for shutdown, never `kill` — the world needs to flush. `systemctl --user stop mc` does exactly this and waits for it; that is the whole reason the unit exists, and it is the preferred way.
...
- **Nothing in `BotExecutor` is a stub any more.** `flee` was the last one and landed 2026-09-14 (Task 6b), once Ricky pinned its distance and timeout — **100 blocks as a TARGET, bounded by 10s**, spec §7 Decision 4. The contract suite's `flee` guarantee was the single SKIP in the integration run and **there are now no skips at all**. `placeBlock`, `followPlayer`, `attack`, `buildSchematic` and the `ReflexExecutor` arbiter all shipped earlier in Phase 5 Track A. Every action checks `opts?.signal?.aborted` first and returns `interrupted` — the contract suite asserts this for all eight abortable cases across seven actions (`contract-suite.ts:302-330` — the `abortableActions` table and its `it.each`), and `runAction()` gives it for free; do not re-add it by hand.
