...
| **`npm run demo` (Phase 1) rots between runs.** `cli.ts` targets *current position + 8 in x, at the same y*, against a persistent world in which the bot's position carries over | MEASURED 2026-09-12, two consecutive runs: from (74.1, 72, -4.5) it reported `unreachable` — *"no path to the target"* — and left the bot displaced at (77.3, 71, -3.0); the **very next run**, starting from there, targeted x=85 and **arrived**. The target therefore depends on wherever the last run left the bot, on real terrain, so a red run says nothing about the code. This is the documented "tests that rot" trap living in a demo rather than a test. `demo:phase2`…`demo:phase5` each pin their own arena and do not have it. (It also shows the bounded `searchRadius` working: an honest `NoPath` rather than a `thinkTimeout` burn) |
