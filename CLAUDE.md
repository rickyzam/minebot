...
| **`setTimeout(fn, Infinity)` fires after ~2ms in Node** — the delay overflows and clamps, with only a `TimeoutOverflowWarning` | `runAction` arms its timer unconditionally, so an action called with `timeoutMs: Infinity` times out almost at once. Never express "no timeout" as `Infinity`; skip arming the timer. Found while planning `followPlayer`, whose agreed contract has no default timeout |
| `mineflayer-pathfinder`'s `Movements` defaults **`allow1by1towers = true`** with `scafoldingBlocks = [dirt, cobblestone]`, and the executor overrides only `canDig` | Movement does not dig, but it **can build**: a bot carrying dirt or cobblestone may pillar while pathing and leave towers behind. Never bitten, because no test, demo or script has ever given a bot either block. `placeBlock` will — it could spend the block it was told to place |
...
- `placeBlock`, `followPlayer`, `attack` and `flee` are stubs returning `fail('internal', '… arrives in Phase 5')`. They **do** still check `opts?.signal?.aborted` first and return `interrupted` — the contract suite asserts this for all eight abortable cases, across seven actions (`contract-suite.ts:267-285`). `runAction()` now gives them that check for free; do not re-add it by hand.
